var solveNQueens = function (n) {
    const borad = new Array(n);
    for (let i = 0; i < n; i++) {   // 棋盘的初始化
        borad[i] = new Array(n).fill('.');
    }
    const res = [];
    const isValid = (row, col) => {
        for (let i = 0; i < row; i++) {         // 之前的行
            for (let j = 0; j < n; j++) {       // 所有的列
                if (borad[i][j] == 'Q' &&       // 发现了皇后，并且和自己同列/对角线
    ...
    const helper = (row) => {                   // 放置当前行的皇后
        if (row == n) {                         // 递归的出口，超出了最后一行
            const stringsBoard = borad.slice(); // 拷贝一份boerd
            for (let i = 0; i < n; i++) {
        ...
        for (let col = 0; col < n; col++) {     // 枚举出所有选择
            if (isValid(row, col)) {            // 剪掉无效的选择
                borad[row][col] = "Q";          // 作出选择，放置皇后
                helper(row + 1);                // 继续选择，往下递归
                borad[row][col] = '.';          // 撤销当前选择
            }
        }
    };
    helper(0);                                  // 从第0行开始放置
